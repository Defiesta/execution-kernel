...
///
/// **Important**: This means `AgentOutput` represents an unordered set of actions
/// for commitment purposes. The constraint engine evaluates actions in this
...
mod tests {
    ...
    #[test]
    fn test_agent_output_sorted_encoding() {
        // Create actions in reverse canonical order
        ...
        // Verify encoding is sorted by inspecting the first action in the encoded bytes.
        // Note: decode does NOT sort - it preserves wire order. Encoding sorts.
        let mut offset = 0;
        ...
        // action_b (type 0x01) should be first in canonical order
        assert_eq!(first_action.action_type, 0x01);
        assert_eq!(first_action.target, [0x22; 32]);
    }

    #[test]
    fn test_agent_output_decode_preserves_wire_order() {
        // Verify that decode does NOT re-sort - it preserves the order from the wire.
        // This is important: canonicalization is an encoding step, not a decoding step.
        let action_a = ActionV1 {
            action_type: 0x02,
            target: [0x11; 32],
            payload: vec![],
        ...
        // encode() will sort: [action_b, action_a]
        let encoded = output.encode().unwrap();
        let decoded = AgentOutput::decode(&encoded).unwrap();

        // decoded order matches wire order (which is sorted), not original order
        assert_eq!(decoded.actions.len(), 2);
        assert_eq!(decoded.actions[0].action_type, 0x01); // was second, now first after sorting
        assert_eq!(decoded.actions[1].action_type, 0x02);
    }
}
