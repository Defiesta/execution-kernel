...
fn main() {
    ...
    writeln!(file, "//").unwrap();
    writeln!(file, "// This hash uniquely identifies the example-yield-agent source code.").unwrap();
    writeln!(file, "// It is computed as SHA256(src/lib.rs || 0x00 || Cargo.toml).").unwrap();
    writeln!(file).unwrap();
    writeln!(
        file,
        "/// SHA-256 hash of the yield agent source code."
    )
    .unwrap();
    writeln!(file, "///").unwrap();
    ...
    println!("cargo:warning=AGENT_CODE_HASH (example-yield-agent): {}", hash_hex);
}
