# Canonical Codec and Commitment Specification
...
1. `protocol_version` MUST equal `PROTOCOL_VERSION` (currently 1)
2. `kernel_version` MUST equal `KERNEL_VERSION` (currently 1)
3. `execution_status` MUST equal 0x01 (Success) or 0x02 (Failure)
...
| 0x02 | Failure | Constraint violation (P0.3+) |
| 0x03-0xFF | Reserved | Invalid (reserved for future expansion) |

### Rationale

The value 0x00 is explicitly reserved to detect bugs where uninitialized memory is accidentally interpreted as a valid status.

**P0.1-P0.2:** Only 0x01 (Success) was valid. Failures caused the kernel to abort before committing a journal.

**P0.3+:** Both 0x01 (Success) and 0x02 (Failure) are valid. Constraint violations produce a Failure journal with an empty action commitment. This allows constraint violations to be provable on-chain while still rejecting invalid state transitions.

Decoders MUST reject any value other than 0x01 or 0x02.
...
| `InvalidExecutionStatus` | Status byte is not 0x01 or 0x02 |
...
- Invalid enumeration values (execution_status not in {0x01, 0x02})
